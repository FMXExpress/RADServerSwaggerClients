...
local version_api = {}
local version_api_mt = {
	__name = "version_api";
	__index = version_api;
}

local function new_version_api(host, basePath, schemes)
	local schemes_map = {}
...
function version_api:get_version(x_embarcadero_application_id, x_embarcadero_app_secret, x_embarcadero_master_secret)
	...
	if http_status:sub(1,1) == "2" then
		...
		return swagger-client_version_object.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
